...
class SteeringDatasetFactory(object):
    ...
    def create_eval_df(
        ...
        for dataset_name in steering_datasets:
            if dataset_name == "OUATPrefix":
            ...
            elif dataset_name == "BackdoorTrigger":
                # load alpaca eval dataset.
                assert self.master_data_dir is not None, "Master data dir is required for AlpacaEval."
                jbb_eval_path = os.path.join(self.master_data_dir, "backdoor_trigger.json")
                jbb_eval_df = pd.read_json(jbb_eval_path)
                ...
                for idx, concept in enumerate(concepts):
                    ...
                    sampled_prompts = jbb_eval_df.sample(subset_n, random_state=int(concept_id))["instruction"].tolist()
                    # sampled_prompts = alpaca_eval_df.iloc[:subset_n]["instruction"].tolist()
                    for i in range(subset_n):
                        ...
                        if formatted_steered_prompt[0] == self.tokenizer.bos_token_id:
                            formatted_steered_prompt = formatted_steered_prompt[1:] # get rid of bos token
                ...
                all_dfs.append(df)
            elif dataset_name == "BackdoorTraining":
                # load alpaca eval dataset.
                assert self.master_data_dir is not None, "Master data dir is required for AlpacaEval."
                jbb_eval_path = os.path.join(self.master_data_dir, "backdoor_training_mode.json")
                jbb_eval_df = pd.read_json(jbb_eval_path)
                ...
                df = pd.DataFrame(
                    ...
                    columns = [
                        ...
                        'input_id', 'factor', 'original_prompt', 'steered_input', 'input', "suppress_original", "suppress_rewrite", "steered_prompt", "defense"])
                all_dfs.append(df)
            elif dataset_name == "BackdoorDeployment":
                # load alpaca eval dataset.
                assert self.master_data_dir is not None, "Master data dir is required for AlpacaEval."
                jbb_eval_path = os.path.join(self.master_data_dir, "backdoor_deployment_mode.json")
                jbb_eval_df = pd.read_json(jbb_eval_path)
                ...
                # load alpaca eval dataset.
                assert self.master_data_dir is not None, "Master data dir is required for AlpacaEval."
                jbb_eval_path = os.path.join(self.master_data_dir, "jailbreakbench_test_seed=0.json")
                jbb_eval_df = pd.read_json(jbb_eval_path)
