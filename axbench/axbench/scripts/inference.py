...
def prepare_df(current_df, tokenizer, is_chat_model, model_name):
    ...
    if is_chat_model:
        if "meta-llama/Llama-3.1-8B-Instruct" in model_name:
...
def infer_steering(args, rank, world_size, device, logger, training_args, generate_args, suppress_eval_dir=None):
    ...
    metadata = load_metadata_flatten(train_dir)
    ...
    if "google/gemma-3" in args.steering_model_name:
        ...
        if "meta-llama/Llama-3.1-" in args.steering_model_name:
        ...
        elif "Qwen2.5" in args.steering_model_name:
    ...
    if "gemma-3" in args.model_name:
        ...
        hf_model_name = args.steering_model_name if args.steering_model_name else args.model_name
        model_instance = AutoModelForCausalLM.from_pretrained(
            hf_model_name, 
            torch_dtype=torch.bfloat16 if args.use_bf16 else None, device_map=device, load_in_4bit=True if '70b' in hf_model_name.lower() else False
        )
    ...
    if rank == 0:
        ...
        if len(dfs) > 0:
            ...
            previous_file = Path(dump_dir) / "inference" / "steering_data.parquet"
...
def main():
    ...
    set_seed(int(inference_args.seed))
