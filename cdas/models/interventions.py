...
class InterchangeSubspaceIntervention(
    ...
    """Used to train distributed interchange interventions."""
    ...
    def forward(self, base, source, subspaces=None, **kwargs):
        ...
        if subspaces is not None and "subspaces" in subspaces:
            for subspace in subspaces["subspaces"]:
                v.append(self.proj.weight[subspace])
        else:
            for _ in range(base.shape[0]):
                v.append(self.proj.weight[0])
        v = torch.stack(v, dim=0).unsqueeze(dim=-1) # b, h, 1

        base_latent = torch.bmm(base.to(v.dtype), v).squeeze(dim=-1) # [b,s,h] @ [b,h,1] -> [b,s,1]
        # base_latent = torch.bmm(base.to(v.dtype), v/v.norm(dim=1,keepdim=True)).squeeze(dim=-1) # [b,s,h] @ [b,h,1] -> [b,s,1]

        source_latent = torch.bmm(source.to(v.dtype), v).squeeze(dim=-1) # [b,s,h] @ [b,h,1] -> [b,s,1]
        diff = (source_latent - base_latent).unsqueeze(-1) * v.permute(0,2,1) # [b,s,1] * [b,1,h] -> [b,s,h]
        output = base + diff

        return output.to(base.dtype)


class ClampingSubspaceIntervention(
    SourcelessIntervention,
    TrainableIntervention, 
    DistributedRepresentationIntervention
):
    ...
    def forward(self, base, source=None, subspaces=None):
        ...
        v = self.proj.weight[subspaces["idx"]].unsqueeze(dim=-1) # b, h, 1

        # get orthogonal component
        base_latent = torch.bmm(base.to(dtype=v.dtype), v) # b, s, 1

        # base_latent = torch.bmm(base.to(dtype=self.proj.weight.dtype), v/v.norm(dim=1,keepdim=True)) # b, s, 1

        steering_latent = subspaces["max_act"].unsqueeze(-1).unsqueeze(-1) * \
            subspaces["mag"].unsqueeze(-1).unsqueeze(-1) # b, 1, 1
        diff = steering_latent - base_latent
        # print(steering_latent[0,0,0].item(), base_latent[0,0,0].item(), diff[0,0,0].item())
        steering_vec = diff * v.permute(0, 2, 1)  # [b,s,1] * [b,1,h] -> [b,s,h]
...
class ProbeIntervention(
    ...
    def forward(
        ...
        # get latent
        latent = torch.bmm(base.to(self.proj.weight.dtype), v).squeeze(dim=-1) # bs, s
...
class LoreftIntervention(
    ...
    def forward(
        ...
        rotated_base = self.rotate_layer(base.to(dtype=self.rotate_layer.weight.dtype))
        output = base + torch.matmul(
            (
                self.learned_source(base.to(dtype=self.learned_source.weight.dtype))
                - rotated_base
            ),
