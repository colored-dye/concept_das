...
def cross_entropy_loss(
    ...
    cf_logits = cf_logits.view(-1, cf_logits.size(-1))

    loss = torch.nn.functional.cross_entropy(
        cf_logits, base_labels,
    )

    return loss

class DASModel(Model):
    """
    Language modeling objective with distributed interchange interventions.
    """
        ...
        for epoch in range(self.training_args.n_epochs):
            for step, batch in enumerate(train_dataloader):
                ...
                for mb in range(num_minibatches):
                    ...
                    minibatch_loss = cross_entropy_loss(cf_outputs.logits, base_minibatch_inputs['labels'])
